...
class CPDDatasets:
    ...
    def __init__(
        ...
        random_seed: int = 123,
        train_anomaly_num: Optional[int] = None,
...
class YAHOODataset(Dataset):
    ...
    def __init__(
        ...
        # if balanced:
        #     abnormal_num, normal_num = 0, 0
        #     for cp_idx in self.cp_idxs:
        #         if cp_idx > 0:
        #             abnormal_num += 1
        #         else:
        #             normal_num += 1 
        #     num_samples_per_class = min(normal_num, abnormal_num)
            
        #     np.random.seed(42)
        #     idxs = np.random.choice(np.arange(len(self.data)), num_samples_per_class, replace=False)
        #     self.data, self.cp_idxs = self.data[idxs], self.cp_idxs[idxs]
    ...
    def __getitem__(self, idx: int) -> Tuple[np.array, np.array]:
        ...
        # crop test sequences around change point for seq2seq model evaluation
        if self.seq_len:
            if labels.sum() == 0:
                #start_idx = 0
                start_idx = torch.randint(0, len(labels) - self.seq_len, (1, )).item()
            else:
                # print("cp idx:", cp_idx)
                #start_idx = max(0, cp_idx - w)
                #start_idx = min(start_idx, len(labels) - self.seq_len )
                
                start_idx = max(0, cp_idx - 9 * self.seq_len // 10)
                start_idx = torch.randint(start_idx, cp_idx, (1, )).item()
                start_idx = min(start_idx, len(labels) - self.seq_len)
                
            end_idx = start_idx + self.seq_len            
            sequence, labels = sequence[start_idx:end_idx], labels[start_idx:end_idx]
